    ...
    <div class="flex-grow flex justify-between items-center gap-2">
      <div v-if="expense.type === 'expense'" class="flex flex-col">
        <div class="gap-1 flex items-center pb-1">
          ...
          <span class="text-sm text-primary-600/80 dark:text-primary-300/80">&middot;
            {{ moment.utc(expense.created_at).local().format("MMM DD • hh:mm A") }}
          </span>
        </div>
        <span v-for="[payer, val] in Object.entries(expense.payers)" class="text-sm font-light">{{
      ...
      <span :class="['text-xl', getColorForValue(value)]">{{ useGroups().getGroupCurrency(groupID) }}{{ Math.abs(value)
      }}</span>
    </div>
  </div>
</template>
